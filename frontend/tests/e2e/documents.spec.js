...
test.describe("/documents", () => {
    test("muestra el espacio documental autenticado", async ({ page }) => {
        ...
        await expect(page.getByRole("heading", { name: "Nueva carga documental" })).toBeVisible();
        await expect(page.getByText("Arrastra un archivo aquí o haz clic para seleccionarlo")).toBeVisible();
        ...
        await expect(page.getByRole("button", { name: "Subir y analizar" })).toBeDisabled();
        await expect(page.getByRole("heading", { name: "Últimos documentos" })).toBeVisible();
        await expect(page.getByRole("link", { name: "Abrir listado documental" })).toBeVisible();
        await expect(page.getByRole("heading", { name: "Detalle procesado" })).toBeVisible();
    });
});
